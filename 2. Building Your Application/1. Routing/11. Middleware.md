...
`matcher(匹配器)` 配置支持全部的 regex, 所以支持 negative lookaheads(负查找)或字符匹配. 这里有一个 negative lookaheads(负查找)匹配除特定路径外所有路径的示例：
...
- ``
- 为 API Routes(API 路由), `getServerSideProps` 和 `rewrite` destination(重写目的地)设置请求标头
- 设置返回的 cookies
- 设置返回的 headers

要在 Middleware(中间件) 中制造一个返回, 你可以:

1. `rewrite`(重写) 到产生响应的路由( [Page(页面)](https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts)或 [Route Handler(路由处理器)](https://nextjs.org/docs/app/building-your-application/routing/route-handlers))
...
您可以通过返回 `Response` 或 `NextResponse` 实例直接从中间件作出响应.
...
在 Next.js `v13.1` 中, `skipMiddlewareUrlNormalize` 和 `skipTrailingSlashRedirect`标记被引入中间件中, 用来处理高级的用户案例.
