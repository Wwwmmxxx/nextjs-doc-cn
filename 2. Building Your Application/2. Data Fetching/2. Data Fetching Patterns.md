...
在 React 和 Next.js 中对 fetching data(获取数据) 有一些推荐模式和最佳实践. 本页将介绍一些最常见的模式以及如何使用它们.

## Fetching Data on the Server (在服务端获取数据)
...
通过 Server Components(服务端组件) 和 [nested layouts(嵌套布局)](https://nextjs.org/docs/app/building-your-application/routing/pages-and-layouts), 你可以立即呈现页面中不需要数据的部分, 而且, 你可以对页面中需要获取数据的部分展示一个 [loading state(加载状态)](https://nextjs.org/docs/app/building-your-application/routing/loading-ui-and-streaming). 这意味着, 用户不需要等待整个页面全部加载完成后才能看到页面就可以与页面产生交互.
